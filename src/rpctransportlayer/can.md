...
transmit their ID (and other bits) during arbitration phase and the one with
lowest ID wins the time slot so it can send its data (not ultimately true if
extended ID is used, but SHV uses 11 bits only).
...
aware that the same message can be received multiple times which is valid). Such
message is just lost and no error should be reported for that. This behavior can
be also used for message abort (intentional sequence breaking).

### Flow Control

CAN-FD throughput can be expected to be low enough that even low end devices
should have enough time to handle all data. The issue might rise if multiple
connections are to be forwarded to connection that can't be multiplexed (such as
[stream transport](./stream.md)) which happens in RPC Brokers. To solve this a
simple flow control is part of SHV CAN-FD. The peer can send first data frame
but before continuing sending rest of the message and/or the next first data
frame it must get acknowledgment. This acknowledgment is message with only
destination byte and copy of the second byte from the first data frame.
Important point is that only first frame of the message is confirmed, not the
subsequent ones.
...
------                 --------            ------                     --------
   ...
   |<----------------------|                 |<--------------------------|
   |                       |                             ...
   | Subsequent data frame |                 | First data frame          |
   |---------------------->|                 |-------------------------->|
            ...                                          ...
   | Last data frame       |           
   |---------------------->|           
